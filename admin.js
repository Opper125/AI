...
const _0x4a2b = [
  "aHR0cHM6Ly9leW5iY3BrcHd6aWt3dGxyZGx6YS5zdXBhYmFzZS5jbw==",
  "",
]

const SUPABASE_URL = atob(_0x4a2b[0])
const SUPABASE_ANON_KEY = atob(_0x4a2b[1])

// Initialize Supabase with secure configuration
const supabase = window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY, {
  auth: { persistSession: false },
  global: { headers: { "x-client-info": "secure-admin-v1" } },
})
...
async function getDeviceIP() {
  try {
    ...
    console.error("[v0] IP fetch failed:", error)
    return "unknown"
  }
}
...
function storeSecureSession(token, fingerprint) {
  const sessionData = {
    t: token,
    f: fingerprint,
    ts: Date.now(),
  ...
  localStorage.setItem("_sas", encoded)
  secureSession = {
    token: token,
...
function getSecureSession() {
  try {
    ...
    return {
      token: sessionData.t,
      fingerprint: sessionData.f,
    }
  } catch (error) {
    console.error("[v0] Session retrieval failed")
    return null
  }
}
...
async function checkAdminAuth() {
  ...
  if (session.fingerprint !== currentFingerprint) {
    console.warn("[v0] Device fingerprint mismatch")
    clearSecureSession()
    showLogin()
    return
  }
...
async function adminLogin(event) {
  ...
  try {
    ...
    const browserInfo = JSON.stringify(getBrowserInfo())

    // Verify password and device authorization
    const { data, error } = await supabase.rpc("verify_admin_with_device", {
    ...
    if (result.is_authorized) {
      ...
      storeSecureSession(result.session_token, deviceFingerprint)
      ...
      await supabase.rpc("log_security_event", {
        ...
        ip: deviceIP,
        success: true,
        msg: result.is_first_login ? "First login" : "Returning login",
      })

      showSuccess(errorEl, result.message + " Redirecting...")
...
async function changeAdminPassword() {
  ...
  try {
    ...
    // Update password with device verification
    ...
    if (result.success) {
      hideLoading()
      alert(result.message + " You will be logged out.")
      clearSecureSession()
      location.reload()
    } else {
      hideLoading()
...
async function uploadFile(file, folder) {
  try {
    ...
    const { data, error } = await supabase.storage.from("website-assets").upload(fileName, file, {
      cacheControl: "3600",
      upsert: false,
    })
...
async function loadAnimations() {
  try {
    ...
    const { data, error } = await supabase.from("animations").select("*").order("created_at", { ascending: false })
    ...
    console.error("[v0] Error loading animations:", error)
    allAnimations = []
  }
}
...
function displayAnimations(animations) {
  ...
  if (animations.length === 0) {
    container.innerHTML =
      '<p style="text-align:center;color:#94a3b8;padding:40px;">No animations yet. Upload your first animation!</p>'
    return
  }
...
function openEmojiPicker(inputId) {
  ...
  if (allAnimations.length === 0) {
    grid.innerHTML =
      '<p style="text-align:center;color:#94a3b8;padding:40px;">No animations available. Upload some first!</p>'
  } else {
...
async function updateSettings(updates) {
  try {
    const { error } = await supabase.from("website_settings").update(updates).eq("id", websiteSettings.id)
...
async function addBanner() {
  ...
  if (url) {
    try {
      const { error } = await supabase.from("banners").insert([{ image_url: url }])
...
async function addCategory() {
  ...
  showLoading()
  try {
    const { error } = await supabase.from("categories").insert([{ title: title }])
...
async function editCategory(id, currentTitle) {
  ...
  showLoading()
  try {
    const { error } = await supabase.from("categories").update({ title: newTitle }).eq("id", id)
...
async function loadCategoriesForSelect() {
  try {
    ...
    const selects = ["buttonCategorySelect", "tableCategorySelect", "menuCategorySelect", "videoCategorySelect"]
...
async function loadCategoryButtons() {
  try {
    ...
    console.error("[v0] Error loading buttons:", error)
  }
}
...
async function editButton(id) {
  const { data: button } = await supabase.from("category_buttons").select("*").eq("id", id).single()
...
async function updateButton(id) {
  ...
  showLoading()
  try {
    const { error } = await supabase.from("category_buttons").update({ name: name }).eq("id", id)
...
async function loadButtonsForTables() {
  ...
  try {
    const { data } = await supabase.from("category_buttons").select("*").eq("category_id", categoryId)
...
async function saveTables() {
  ...
  showLoading()
  try {
    const { error } = await supabase.from("input_tables").insert(tables)
...
async function deleteTable(id) {
  ...
  showLoading()
  try {
    const { error } = await supabase.from("input_tables").delete().eq("id", id)
...
async function loadMenus() {
  try {
    ...
    console.error("[v0] Error loading menus:", error)
  }
}
...
async function editPayment(id) {
  const { data: payment } = await supabase.from("payment_methods").select("*").eq("id", id).single()
...
async function addAdminMessage(id) {
  ...
  showLoading()
  try {
    const { error } = await supabase.from("orders").update({ admin_message: message }).eq("id", id)
...
async function loadUsers() {
  try {
    ...
    if (data && data.length > 0) {
      ...
      const todayUsers = data.filter((user) => new Date(user.created_at).toDateString() === today).length
